#Stuart- Well written for the most part, but a few small errors. Did your computer not
# give you an error when trying to run plot.figure()?
...
#Stuart- The keyword was names, not columnnames. Received an error when trying to run it as originally written
...
#Stuart- You need to define the columns list before you call on it in the sentence below.

#For the next data set, we want to read the abalone data and assign column names
abalone=pd.read_csv("http://archive.ics.uci.edu/ml/machine-learning-databases/abalone/abalone.data", names = abalone_columns)
...
def optimalbinwidth(a, b, c, r):
    ...
    """
#Stuart- Your binwidth as calculated below can be negative (which is not allowed).
# Did you mean to use b-c?
