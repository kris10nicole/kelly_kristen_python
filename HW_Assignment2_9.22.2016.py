...
"""Homework Problem 4
The purpose of the function is to calculate the length of the recurring cycle for the reciprocal of an
integer in the decimal representation. Then, it will find the number that produces the longest length.
"""

from decimal import * #Here we want to import all the formulas from the decimal module
getcontext().prec = 2000 #Here we want to set a longer decimal place for our calculations 
def recur_cycle(value): #Here we define the function recurring_cycle and assign it the parameter value
    tem = Decimal(1)/Decimal(value) #Here we get the decimal numbers 
    deci=list(str(tem)) #Here we transfer the decimal to a list to use later for comparison
    if len(deci)<999: #Here we check to see if the decimal places is finite...
        return 0 #...if it is, then the length of recursion cycle is 0 
    else: 
        n=1 #Otherwise the length of the recursion cycle is one
        while True:     
            if deci[10]==deci[10+n]: #Here we compare the value from the 10th term because some cycles do not start from the beginning (so we define the start from 10th value); if a value is the same as the start, turn it into the next comparison  
                z=1 # Here we make z a stop indicator, if n is cycle length, then z=1, otherwise z=0 
                for i in range(0,n):    #Here we check the value from the previous digits to see if they cycle 
                    if deci[10+i]==deci[10+n+i]:#if one value is the same at the beginning, then compare all the values after that position 
                        continue 
                    else: 
                        z=0 #If z =0, n is not a valid cycle length 
                        break 
                    if z==1: 
                        return n  #If z =1, the range up to n is a cycle, and we return the cycle length n 
                    else: 
                        n=n+1 #If n is not cycle length, we want to increment n by 1 to check for recurring cycle  
                        continue 
            else:    
                n=n+1 
                continue 

"""Here we use the function recur_cycle() to find the value of d < 500 for which 1/d contains the
longest recurring cycle in its decimal fraction part 
"""
def main(): 
        value=2 #value is a parameter for recur_cycle()
        max=recur_cycle(1)   #Here we initialize the max length 1 
        for i in range(2,500): #Here we check the number from 2 to 500 
            if recur_cycle(i)>max:  #If current length of the cycle is longer than the max, update it as the new max 
                max=recur_cycle(i)  
                value=i         #Here we want to store the length of the variable as value 
            else: 
                continue             
print('The number', value, 'has the longest recurring cycle with cycle length: ',max) #Max is the maximum length of recursion cycle and 'value' is its corresponding value 
main() #Here we run the function
...
def calculation(x, n): #Here we define the function calculation and assign it the parameters x and n
    e1 = x #Here we assign a new variable e1 to x (e1 will be used later)
    e2 = x #Here we assign a new variable e2 to x (e2 will be used later)
    e3 = x #Here we assign a new variable e3 to x (e3 will be used later)
    i  = 0 
    while i < n: 
        e1 = 3.95 * e1 * (1 - e1) #Here we assign a formula to calculate the value for e1
        e2 = 3.95 * e2 - 3.95 * e2 ** 2 #Here we assign a formula to calculate the value for e2
        e3 = 3.95 * (e3 - e3 ** 2) #Here we assign a formula to calculate the value for e3
        print(e1, e2, e3) #We want to print the results of e1, e2, and e3
        i += 1 

calculation(.9, 100) #Here is an example calculation
